...
if strcmp(version,'sample_mean')
    ...
    mesh_size_t = 5; %5;  % torso
    mesh_size_i = 15; %15;  % interior
    refine_grad = 0.3;  % slow=0.1, fast=0.3
    path = strcat('geom/',pca_id,'/',version_id,'/mesh_seeds/');
    pv_file_t = strcat('mesh_seeds_torso_',sprintf('%04d',mesh_size_t),'.csv');
    mesh_lungs = false;

elseif strcmp(version,'truth')
    version_id = strcat('truth_', subject_id);
    mesh_size_t = 5; %2.5;  % torso
    mesh_size_i = 10; %5;  % interior
    refine_grad = 0.3;  % slow=0.1, fast=0.3
    path = strcat('geom/',pca_id,'/',version_id,'/mesh_seeds/');
    pv_file_t = strcat('mesh_seeds_torso_',sprintf('%04d',mesh_size_t),'.csv');
    pv_file_l = strcat('mesh_seeds_lung_left_',sprintf('%04d',mesh_size_i),'.csv');
    pv_file_r = strcat('mesh_seeds_lung_right_',sprintf('%04d',mesh_size_i),'.csv');
    mesh_lungs = true;
...
elseif strcmp(version,'predicted')
    ...
    refine_grad = 0.1;  % slow=0.1, fast=0.3
    path = strcat('geom/',pca_id,'/',version_id,'/mesh_seeds/');
    pv_file_t = strcat('mesh_seeds_torso_',sprintf('%04d',mesh_size_t),'.csv');
    mesh_lungs = false;
end
...
save_file = strcat('trimesh.mat');
if ~exist(save_path, 'dir'), mkdir(save_path), end
save(strcat(save_path,save_file),'tgeom')
save_file_nodes = strcat('trimesh_nodes.csv');
writematrix(tgeom.Points, strcat(save_path,save_file_nodes))
